...
_Last updated: 2026-06-26 — **Block 14 (Watchlist) is COMPLETE and smoke-tested, but NOT yet committed.** Migration `0004_business_watchlist.sql` has been applied + verified in Supabase (all checks a–g pass). The app layer (per-row star toggle on `/database`, new `/watchlist` route + nav item reusing the Database list UI via an `onlyIds` filter, add/remove server actions) is built; `tsc` + `build` clean; full 1–14 smoke test PASSED. **Awaiting commit/push approval.** Block 13 (Member Invites) was committed + pushed at HEAD `1076219`. Block 12 stays fallback-chip-only (`LOGO_ALLOWLIST` empty). See §2._
...
- **Completed:** Block 1 (App Foundation), Block 2 (Authentication), Block 3 (Workspace Architecture), Block 4 (Schema / Business Data), Block 5 (Search / Filter / Sort), Block 6 (CSV Import), Block 7 (Duplicate Detection — read-only), Block 8 (Safe Manual Duplicate Merge UI), Block 9 (Restore / Archive UI), Block 10 (Database UI/UX polish), Block 11 (Dashboard metrics / CRM overview), Block 12 (Brand logo chips with fallback), Block 13 (Member Invites / Team Workflow) — **all committed + pushed (HEAD `1076219`).** Block 14 (Watchlist) — **done + smoke-tested, NOT yet committed.**
