...
_Last updated: 2026-06-26 — **Block 13 (Member Invites / Team Workflow) is COMPLETE and smoke-tested, but NOT yet committed.** Migration `0003_workspace_invites.sql` has been applied + verified in Supabase (all checks a–g pass). The app layer (invite create/revoke/accept, `/invite/[token]` route, Settings invite panel, open-redirect-safe login/signup `next` redirect) is built; `tsc` + `build` clean; full 1–15 smoke test PASSED. **Awaiting commit/push approval.** Blocks 1–12 remain committed + pushed (HEAD `cce4da7`). Block 12 stays fallback-chip-only (`LOGO_ALLOWLIST` empty; the extracted PNG experiment was reverted). See §2._
...
- **Completed:** Block 1 (App Foundation), Block 2 (Authentication), Block 3 (Workspace Architecture), Block 4 (Schema / Business Data), Block 5 (Search / Filter / Sort), Block 6 (CSV Import), Block 7 (Duplicate Detection — read-only), Block 8 (Safe Manual Duplicate Merge UI), Block 9 (Restore / Archive UI), Block 10 (Database UI/UX polish), Block 11 (Dashboard metrics / CRM overview), Block 12 (Brand logo chips with fallback) — **all committed + pushed (HEAD `cce4da7`).** Block 13 (Member Invites) — **done + smoke-tested, NOT yet committed.**
...
- **Next up:** **commit + push Block 13** (awaiting approval), then **Block 14 — not yet scoped.** Deferred backlog: Google Maps scraper CSV preset, AI cleaning/OCR, bulk restore, merge history / field-level merge undo. **Block 13 follow-ups (deferred):** real email delivery of invite links, a profiles/identity table (so members show real emails, not "Member <id>"), bulk invites, resend/rotate token, ownership transfer. **Block 12 follow-up:** add real local brand-logo SVGs under `public/brand-logos/` and register their slugs in `LOGO_ALLOWLIST`; chips in forms / import review / merge sheet remain deferred. Dashboard follow-ups (deferred): exact top-N at scale (needs an RPC/SQL view → stop and ask), date-range / trend metrics, clickable status cards that deep-link into `/database?status=…`.
